...
shinyServer(function(input, output, session) {
  ...
  observeEvent(input$newgame, {
    ...
    shinyalert(
      ...
      callbackR = function(x) { if(x == 'Erase') {
            ...
            # Reset status
            write.table("new", 'status.csv', row.names = FALSE, col.names = 'Status', quote = FALSE, sep = ",")
            ...
            set.seed(Sys.time())
            id <- sample(1:99999, 1)
            write.csv(id,'seed.csv', row.names = FALSE, col.names = 'Seed', quote = FALSE)
            }
        },
    )
  })
  ...
  output$infoToMaster2 <- renderText({
    out <- paste("<ul>",
                 "<li><em>The table below is visible to the players.</em></li>",
                 "</ul>")
    out
  })
})
